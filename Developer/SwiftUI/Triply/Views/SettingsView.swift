...
struct SettingsView: View {
    ...
    private var proSection: some View {
        Section {
            HStack {
                Image(systemName: RevenueCatManager.shared.isPro ? "crown.fill" : "crown")
                    .foregroundColor(.yellow)
                Text(RevenueCatManager.shared.isPro ? "Pro Unlocked" : "Unlock Pro")
                    .foregroundColor(.primary)
                Spacer()
            ...
            if !RevenueCatManager.shared.isPro {
                ...
                Button {
                    Task { await RevenueCatManager.shared.restorePurchases() }
                } label: {
            ...
            } else if let err = RevenueCatManager.shared.lastErrorMessage {
...
private struct QuickFeedbackView: View {
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text("Your feedback helps us improve Triply. All feedback is appreciated!")
            }
    ...
    private func formattedEmailText() -> String {
        let subject = "Triply Travel App - User Feedback"
        let body = """
        To: \(recipientEmail)
        Subject: \(subject)
    ...
    private func copyToClipboard() {
        let emailText = """
        To: \(recipientEmail)
        Subject: Triply Travel App - User Feedback
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("1. Select project → Triply target")
