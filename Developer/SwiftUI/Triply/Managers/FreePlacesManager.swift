...
class FreePlacesManager: ObservableObject {
    ...
    func searchNearby(
        ...
        // Try Overpass API first (better for POI searches)
        // Note: Overpass doesn't support pagination, so we get all results
        let overpassResults = await searchWithOverpassAPI(location: location, radius: radius, category: category, query: query)
        
        // If we got results, return them (Overpass doesn't support pagination)
        if !overpassResults.isEmpty {
            // For Overpass, we can simulate pagination by slicing results
            if offset > 0 {
                let startIndex = min(offset, overpassResults.count)
                let endIndex = min(offset + limit, overpassResults.count)
                return Array(overpassResults[startIndex..<endIndex])
            }
            return Array(overpassResults.prefix(limit))
        }
        
        // Fallback to Nominatim if Overpass returns no results
        // Nominatim supports pagination via offset parameter
        print("⚠️ Overpass returned no results, trying Nominatim...")
        return await searchWithNominatim(location: location, radius: radius, category: category, query: query, offset: offset, limit: limit)
    }
    ...
    private func searchWithOverpassAPI(
        ...
        request.setValue("Triply Travel App (iOS)", forHTTPHeaderField: "User-Agent")
