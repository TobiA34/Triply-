...
struct WidgetDataLoader {
    ...
    static func loadTrips() -> [TripModel] {
        // Try to create container - if this fails, try UserDefaults fallback
        guard let container = createContainer() else {
            // Container creation failed - try UserDefaults/file fallback
            // This is expected if App Groups isn't configured
            return loadTripsFromUserDefaultsFallback()
        }
        
        // Try to fetch trips from the container
        ...
        do {
            ...
            if !trips.isEmpty {
                print("✅ Widget: Loaded \(trips.count) trip(s) from SwiftData")
            }
            return trips
        } catch {
            // Fetch failed - try UserDefaults fallback
            // This is expected if there's a schema mismatch or other issue
            return loadTripsFromUserDefaultsFallback()
        }
    }
